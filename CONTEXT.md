...
- **Build Scribe** — a separate agent role responsible for blog work. Distinct from Ralph (different trigger, different output, different prompt). Has three modes: Scout, Intake, and Drafter.
- **Scout** — the Build Scribe mode that reads recent work across all projects and produces a backlog of blog ideas. Output: cards appended to `blog-ideas.md`. Picky by design — aims for 8–15 well-formed ideas per project, not exhaustive coverage. Autonomous; never asks clarifying questions.
- **Intake** — the Build Scribe mode that turns a human blog request into a single well-formed card. Interactive: walks Antonio through archetype choice, voice tweaks, audience, source/material, and verdict, then writes one card with `Status: drafting` to `blog-ideas.md`. Distinct from Scout (which is autonomous and reads repos) and Drafter (which writes prose). Used when Antonio knows what he wants to write about and needs the card built around that intent.
- **Drafter** — the Build Scribe mode that turns an approved blog idea into a full Sanity post. Output: a `blogPost` document in Sanity via the Sanity MCP, written as Portable Text JSON. Processes one card per run.
...
- **Blog archetype** — one of four structural patterns: **hybrid** (tradeoff + verdict, the default for developer-targeted posts — matches existing posts), **technical** (problem → solution → verification, code-heavy), **reflective** (opinion or narrative, minimal code), **explainer** (friendly walkthrough of a concept, accessible to non-developers, Julia-voiced).
- **Voice anchor** — the writer Build Scribe imitates for tone, rhythm, and structural habits. **Mitchell Hashimoto** (`mitchellh.com/writing`) is the anchor for `hybrid`, `technical`, and `reflective` — confident, thesis-up-top, developer-targeted, no hedging. Canonical samples: `/writing/ghostty-leaving-github`, `/writing/simdutf-no-libcxx`, `/writing/building-block-economy`, `/writing/my-ai-adoption-journey`, `/writing/tripwire`. **Julia Evans** (`jvns.ca`) is the anchor for `explainer` — friendly, curious, accessible to non-developers, headings-as-questions, analogies for jargon. Canonical samples: `/blog/2018/01/06/operating-systems-2018/`, `/blog/2023/09/29/why-does-0-1-0-2-equal-0-30000000000000004/`, `/blog/2024/01/27/find-a-domain-expert/`.
